  ...
  return [
    {
      ...
      notes: "Buen estado general. Se recomendó mantener rutina de actividad y control preventivo trimestral."
    },
    {
      id: "h_2",
      dateISO: daysFromToday(-45),
      title: "Seguimiento clínico",
      notes: "Consulta de control sin incidentes. Próximo seguimiento sugerido en agenda preventiva."
    },
    {
      id: "h_3",
      dateISO: daysFromToday(-92),
      title: "Consulta dermatológica",
      notes: "Irritación leve resuelta con indicación tópica. Se indicó seguimiento preventivo."
    }
  ];
}
...
export default function PortalPacienteClientPage({
  portalTitle = "Mi Perfil",
  portalDescription = "Una vista clara del portal del paciente con estado general, próximos cuidados e historial clínico esencial."
}: PortalPageProps) {
  ...
  function removeTrackingItem(id: string) {
  ...
  const waText = `Hola. Quiero coordinar seguimiento para ${pet.petName}.\nPerfil: ${pet.species}.\nControles próximos: ${pet.vaccines.filter(v => soon(v.nextDueISO)).map(v => `${v.name} (${v.nextDueISO})`).join(", ") || "N/A"}`;

  return (
    <Container className="py-10">
      <SectionHeading eyebrow="Portal" title={portalTitle} desc={portalDescription} />
        ...
        <Card className="lg:col-span-3">
          <CardHeader className="flex items-center justify-between">
            <div className="grid">
              ...
              <div className="text-sm text-black/60">Resumen general y seguimiento preventivo</div>
            </div>
            {hasOverdue ? <Badge tone="bad">Hay seguimientos vencidos</Badge> : dueSoonCount > 0 ? <Badge tone="warn">Requiere seguimiento</Badge> : <Badge tone="good">Estado general estable</Badge>}
          </CardHeader>
          <CardContent className="grid gap-4">
            <div className="grid gap-3 sm:grid-cols-4">
              ...
              <div className="rounded-2xl border border-black/10 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-wide text-black/50">Próximo control</div><div className="mt-1 text-sm font-extrabold">{nextItem?.name ?? "Sin pendientes"}</div><div className="text-xs text-black/55">{nextItem?.nextDueISO ? formatDateLong(nextItem.nextDueISO) : hasOverdue ? "Hay seguimientos vencidos para revisar" : "No hay controles cargados"}</div></div>
              <div className="rounded-2xl border border-black/10 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-wide text-black/50">Próximos controles</div><div className="mt-1 text-sm font-extrabold">{dueSoonCount}</div><div className="text-xs text-black/55">en los próximos 30 días</div></div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-black/10 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-wide text-black/50">Programa</div><div className="mt-1 text-sm font-semibold">{pet.breed || "No informado"}</div></div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button onClick={() => window.open(toWhatsAppLink(BRAND.whatsapp, waText), "_blank")} className="bg-cyanSoft-400 text-graphite-950 hover:bg-cyanSoft-300">Solicitar seguimiento por WhatsApp</Button>
              <CommercialImplementationCTA />
            </div>

            <div className="grid gap-2 rounded-2xl border border-black/10 bg-white p-4">
              <div className="text-sm font-extrabold">Seguimiento y próximos controles</div>
              <div className="grid gap-2">
                {pet.vaccines.length === 0 ? (
                  <div className="rounded-2xl border border-black/10 bg-black/[0.02] p-4 text-sm text-black/60">Esta ficha todavía no tiene controles registrados.</div>
                ) : (
                  pet.vaccines.map(v => (
                    <div key={v.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-black/5 px-3 py-2">
                      <div className="grid">
                        <div className="text-sm font-semibold">{v.name}</div>
                        <div className="text-xs text-black/55">Registro: {formatDateLong(v.dateISO)} {v.nextDueISO ? `· Próx.: ${formatDateLong(v.nextDueISO)}` : ""}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        {overdue(v.nextDueISO) ? <Badge tone="bad">Vencido</Badge> : soon(v.nextDueISO) ? <Badge tone="warn">Próximo</Badge> : v.nextDueISO ? <Badge tone="neutral">Vigente</Badge> : <Badge tone="neutral">Sin próximo</Badge>}
                        <button className="rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold hover:bg-black/5" onClick={() => removeTrackingItem(v.id)}>Quitar</button>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="mt-2 grid gap-3 sm:grid-cols-3">
                <Field label="Nombre del control"><Input value={vName} onChange={e => setVName(e.target.value)} placeholder="Ej: Chequeo anual" /></Field>
                <Field label="Fecha"><Input type="date" value={vDate} onChange={e => setVDate(e.target.value)} /></Field>
                <Field label="Próximo seguimiento (opcional)"><Input type="date" value={vNext} onChange={e => setVNext(e.target.value)} /></Field>
              </div>
              <div className="flex flex-wrap gap-2"><Button onClick={addTrackingItem} variant="outline">Agregar control</Button></div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="grid gap-3">
            {history.map(h => (
              <div key={h.id} className="grid gap-1 rounded-2xl border border-black/10 bg-white p-4">
                <div className="flex items-start justify-between gap-3"><div className="text-sm font-extrabold">{h.title}</div><Badge tone="neutral">{h.dateISO}</Badge></div>
                <div className="text-xs text-black/55">{formatDateLong(h.dateISO)}</div>
                <p className="text-sm text-black/70">{h.notes}</p>
              </div>
            ))}

            <div className="text-xs text-black/50">Resumen clínico breve para seguimiento cotidiano del paciente.</div>
          </CardContent>
        </Card>
      </div>
    </Container>
